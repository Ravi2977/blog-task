...
const CREATE_BLOG_API_URL = "http://localhost:5001/api/blogs"; // change to your full backend URL if it's on a different origin
          ...
          <div className="flex gap-1">
            {["left", "center", "right"].map((a) => (
              <button
                ...
                className={`rounded-full px-2.5 py-1 capitalize font-medium transition ${
                  align === a
                    ? "bg-zinc-900 text-white"
                    : "bg-white text-zinc-500 hover:bg-zinc-100"
                }`}
...
function BlockEditor({
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={`group relative flex gap-2 rounded-xl border transition ${
        isDragOver
        ...
        {items.map((item, i) => (
          <li key={item.id}>
            <button
              ...
              className={`flex w-full items-start gap-2 text-left text-sm leading-6 transition ${
                activeId === item.id
              ...
              return (
                <div
                  ...
                  className={`flex flex-col gap-6 sm:flex-row ${
                    b.imagePosition === "right" ? "sm:flex-row-reverse" : ""
                  }`}
                >
                  {b.imageUrl && (
                    <figure
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <select
                ...
                className={`min-w-[180px] flex-1 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-medium outline-none focus:border-zinc-400 ${
                  category ? "text-zinc-700" : "text-zinc-400"
                }`}
              >
